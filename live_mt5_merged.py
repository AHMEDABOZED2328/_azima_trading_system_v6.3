...
# ========== إدارة المخاطر ==========
USE_FIXED_LOT = True       # استخدام لوت ثابت
FIXED_LOT_SIZE = 0.01      # حجم اللوت الثابت المفضل (يمكنك تغييره لاحقاً)
BASE_RISK_PERCENT = 0.001  # 1% (تستخدم فقط في حال تعطيل اللوت الثابت)
MAX_DAILY_LOSS = 0.03  # 3%
MAX_WEEKLY_LOSS = 0.08  # 8%
MAX_OPEN_POSITIONS = 50

# ========== أهداف ونقاط الوقف ==========
USE_DOLLAR_STOPS = True   # True لاستخدام الستوب لوس والهدف بالدولار بدلاً من الاستراتيجية (ATR)
DOLLAR_SL = 15.0          # وقف الخسارة بالدولار (إذا تم تفعيل الخيار أعلاه)
DOLLAR_TP = 10.0         # أخذ الربح بالدولار
...
class RiskManager:
    ...
    def calculate_stops(self, symbol, price, order_type, atr_pips, lot_size=0.01):
        ...
        try:
            ...
            if USE_DOLLAR_STOPS and lot_size > 0:
                contract_size = symbol_info.trade_contract_size
                point = symbol_info.point
                pip_value = contract_size * point * 10
                
                # تحويل الدولار إلى نقاط حسب اللوت
                if pip_value > 0:
                    sl_pips = DOLLAR_SL / (lot_size * pip_value)
                    tp_pips = DOLLAR_TP / (lot_size * pip_value)
                else:
...
def execute_trade(signal, confidence):
    ...
    try:
        ...
        stops = risk_mgr.calculate_stops(SYMBOL, price, order_type, risk_check['atr'], risk_check['lot_size'])
